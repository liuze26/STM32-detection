...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 1://帮助指令
			...
			usb_printf("    USMART是由ALIENTEK开发的一个灵巧的串口调试互交组件,通过 \r\n");
			usb_printf("它,你可以通过串口助手调用程序里面的任何函数,并执行.因此,你可\r\n");
			usb_printf("以随意更改函数的输入参数(支持数字(10/16进制)、字符串、函数入\r\n");	  
			usb_printf("口地址等作为参数),单个函数最多支持10个输入参数,并支持函数返 \r\n");
			usb_printf("回值显示.新增参数显示进制设置功能,新增进制转换功能.\r\n");
			usb_printf("技术支持:www.openedv.com\r\n");
			usb_printf("USMART有7个系统命令:\r\n");
			usb_printf("?:      获取帮助信息\r\n");
			usb_printf("help:   获取帮助信息\r\n");
			usb_printf("list:   可用的函数列表\r\n\n");
			usb_printf("id:     可用函数的ID列表\r\n\n");
			usb_printf("hex:    参数16进制显示,后跟空格+数字即执行进制转换\r\n\n");
			usb_printf("dec:    参数10进制显示,后跟空格+数字即执行进制转换\r\n\n");
			usb_printf("runtime:1,开启函数运行计时;0,关闭函数运行计时;\r\n\n");
			usb_printf("请按照程序编写格式输入函数名及参数并以回车键结束.\r\n");    
			usb_printf("--------------------------ALIENTEK------------------------- \r\n");
#else
			usb_printf("指令失效\r\n");
#endif
...
void usmart_exe(void)
{
	...
	for(i=0;i<pnum;i++)//输出参数
	{
		...
		{
			temp[i]=*(u32*)(usmart_dev.parm+usmart_get_parmpos(i));
			if(usmart_dev.sptype==SP_TYPE_DEC)usb_printf("%lu",temp[i]);//10进制参数显示
			else usb_printf("0X%X",temp[i]);//16进制参数显示 	   
		}
...
//如果非ALIENTEK用户,则USB_USART_RX_STA和USB_USART_RX_BUF[]需要用户自己实现
void usmart_scan(void)
{
	u8 sta,len;  
	if(USB_USART_RX_STA&0x8000)//串口接收完成？
	{					   
		len=USB_USART_RX_STA&0x3fff;	//得到此次接收到的数据长度
		USB_USART_RX_BUF[len]='\0';	//在末尾加入结束符. 
		sta=usmart_dev.cmd_rec(USB_USART_RX_BUF);//得到函数各个信息
